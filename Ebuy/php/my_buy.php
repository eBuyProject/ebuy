<?php
	...
	function my_buy(){
		...
		while($rows = $stmt->fetchObject()){
			...
			if ($row->fldUsername == $_SESSION['username']){
				echo '</td><td>Anzeige entfernt</td>';
			}
			else{
				...
				echo '
					...
					if ($rows->fldFkRating == NULL){
						echo'
							...
							<div id="modalComment' . $rows->fldIdProduct . '" class="modal modalComment">
								<div class="modal-content">
									<p class="Description">Hinterlasse eine Bewertung und ein Kommentar für den Verkäufer</p>
									<br />
		...
		while($rows = $stmt->fetchObject()){
			...
			$sql_sold = "SELECT COUNT(fldIdProduct) FROM tblProducts WHERE fldFkSoldBy LIKE '$rows->fldFkSoldBy'";
			$quantitys = $handle -> prepare($sql_sold);
			$quantitys->execute();
			$row_sold = $quantitys->fetchColumn();
			
			//SQL query to find how many products the seller  has bought
			$sql_boughtd = "SELECT COUNT(fldIdProduct) FROM tblProducts WHERE fldFkBoughtBy LIKE '$rows->fldFkSoldBy'";
					...
					<div class="modal-content">
						...
						<div class="clearfix float-my-children">
							...
							<div>
								...
								<p class="DescVerkaufer">Verkäufer: <a href="userprof.php?user=' . $row->fldIdUser . '">' . $row->fldUsername . '</a></p>
							</div>
						</div>
					</div>
					<div class="modal-footer">
			...
			echo '
				...
				<div id="modalCommentShow' . $rows->fldIdProduct . '" class="modal">
					<div class="modal-content">
						<p class="Description">Kommentar am Verkäufer</p>
						<br />
							<div class="input-field col s12">'
							. $row_c->fldComment .	
							'</div>
